...
attack :- \+ losing, chosenToke(X,_), toke(X,_,Att,_,TypeM), lawan(A,HP,B,C,TypeL),
          ...
          write('Kamu menyebabkan '), write(D), write(' damage pada '), write(A),nl,nl,
          retract(lawan(_,_,_,_,_)), asserta(lawan(A,Z,B,C,TypeL)), cekhealthL, !.   
attack :- \+ losing, chosenToke(X,_), toke(X,_,Att,_,_), lawan(A,HP,B,C,TypeL),
          Z is (HP - Att), 
          write('Kamu menyebabkan '), write(Att), write(' damage pada '), write(A),nl,nl,
          retract(lawan(_,_,_,_,_)), asserta(lawan(A,Z,B,C,TypeL)), cekhealthL, !.

specialAttack :- losing, lose, !.
specialAttack :- \+ losing, chosenToke(X,1), toke(X,_,_,Skill,TypeM), lawan(A,HP,B,C,TypeL),
...
specialAttack :- \+ losing, chosenToke(X,1), toke(X,_,_,Skill,TypeM), lawan(A,HP,B,C,TypeL),
                 ...
                 retract(lawan(_,_,_,_,_)), asserta(lawan(A,Z,B,C,TypeL)),
                 retract(chosenToke(X,1)), asserta(chosenToke(X,0)), cekhealthL, !.
specialAttack :- \+ losing, chosenToke(X,1), toke(X,_,_,Skill,_), lawan(A,HP,B,C,TypeL),
                 Z is (HP - Skill),
                 write('Kamu menyebabkan '), write(Skill), write(' damage pada '), write(A),nl,nl,   
                 retract(lawan(_,_,_,_,_)), asserta(lawan(A,Z,B,C,TypeL)),
                 retract(chosenToke(X,1)), asserta(chosenToke(X,0)), cekhealthL, !.
specialAttack :- \+ losing, \+(chosenToke(X,1)) , write(X), write(' sudah memakai Skill Attack!'), nl.
